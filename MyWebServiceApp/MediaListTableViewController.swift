...
class MediaListTableViewController: UITableViewController {
    ...
    //ตัวแปร Array ที่ใช้รับค่าจาก WebService
    var mySongList : [[String : String]] = []
    var myVideoList : [[String : String]] = []
    //เครื่องเล่นเพลงและวิดีโอ
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt
        ...
        if indexPath.section == 0{
            ...
            let mySong : URL = URL(string:"http://www.worasit.com/iosbuilder/" + myAVItem["fileName"]!)!
                ...
                audioPlayer!.play()
            let alertController = UIAlertController(title: "AVPlayer", message: "กําลังเล่นเพลง " + myAVItem["avName"]! + " " + myAVItem["artistName"]!, preferredStyle: .actionSheet)
            let cancelButton = UIAlertAction(title: "ปิดหน้าต่าง", style: .cancel, handler: nil)
            let pauseButton = UIAlertAction(title: "Pause", style: .default, handler: { (action) in self.audioPlayer!.pause()
            })
            ...
            if let player = moviePlayer{
                NotificationCenter.default.addObserver(self,
                                                       selector: #selector(videoHasFinishedPlaying), name:
